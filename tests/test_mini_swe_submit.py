...
calls and one is the submit (`COMPLETE_TASK_AND_SUBMIT_FINAL_OUTPUT`), the shim
must emit each executed action's real result, close the submit action with the
submission, and emit nothing for actions that never ran.

Gated on ``minisweagent`` (only installed in the sandbox) — skips cleanly where
the runtime is absent, runs where it is present.
"""
...
from benchflow_agents.mini_swe import acp_shim as shim
