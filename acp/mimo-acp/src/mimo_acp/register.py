...
DIRECT-provider mode (base URL present, no alias) the launcher writes no
config and leaves ``OPENAI_*`` intact — the env IS the provider config and
benchflow delivers the model over ACP ``set_model``.
...
_MIMO_PKG = "@mimo-ai/cli@0.1.4"  # pinned: an unpinned float can break ACP on upgrade
# NOTE: pinned @0.1.4 — the current published release, aligned with benchflow
