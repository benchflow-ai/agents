...
| [**mimo**](mimo-acp/) | [MiMo Code](https://mimo.xiaomi.com/mimocode) (Xiaomi, an OpenCode fork) — its `mimo` CLI ships a **native** `mimo acp` server, registered out-of-core ([mimo-acp](mimo-acp/)); no `server.mjs` (mimo *is* the ACP server), structurally like `mini-swe-acp` | ✅ live — `xiaomi/mimo-v2.5-pro` solved `citation-check` (reward 1.0, 17 tools); the free `mimo/mimo-auto` channel runs headless |
