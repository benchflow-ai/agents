...
| [**omnigent**](omnigent/) | [Databricks Omnigent](https://www.databricks.com/blog/introducing-omnigent-meta-harness-combine-control-and-share-your-agents) meta-harness — the first **non-ACP** agents here: ride BenchFlow's Session path via a `session_factory`, shelling `omnigent run` inside the sandbox. `omnigent-pi` (`--harness pi`) **and** `omnigent-mimo` (MiMo Code via an install-time `--harness mimo` overlay + a native-usage trackability bridge) | `omnigent-pi` ✅ reward 1.0 on hello-world **and** the real `citation-check` task (DeepSeek/Daytona x86_64); `omnigent-mimo` 🧪 (key-free tests green; live `mimo/mimo-auto` verification pending). Needs a BenchFlow with the session-factory seam — see [omnigent/README](omnigent/README.md). |
