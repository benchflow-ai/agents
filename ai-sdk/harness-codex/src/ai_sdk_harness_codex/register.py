"""Register the Vercel AI SDK 7 HarnessAgent (Codex harness) with BenchFlow.

⚠️ EXPERIMENTAL / does NOT run as a benchflow eval. The Codex harness is
...
bridge-backed harnesses. For real benchflow evaluation of Codex, use the native
`codex-acp` agent. Shipped for completeness / as a Vercel-sandbox template.
"""
...
_PREFIX = "/opt/benchflow/js-agents/ai-sdk-codex"
_SERVER_SOURCE = (Path(__file__).parent / "server.mjs").read_text()
_DEPS = ("@ai-sdk/harness@canary", "@ai-sdk/harness-codex@canary", "@ai-sdk/sandbox-vercel@canary")
_ALIASES = ("ai-sdk-codex-harness",)


def _install_cmd() -> str:
    b64 = base64.b64encode(_SERVER_SOURCE.encode()).decode()
    pkg = '{"name":"bf-ai-sdk-codex","private":true,"type":"module"}'
    return (
...
def register() -> None:
    """Register the ``ai-sdk-codex`` agent. NOTE: needs a Vercel sandbox to run."""
    register_agent(
        name="ai-sdk-codex",
        install_cmd=_install_cmd(),
        launch_cmd=_launch_cmd(),
        protocol="acp",
        api_protocol="openai-responses",  # Codex speaks the OpenAI Responses API
        env_mapping={
        ...
        description=(
            "Vercel AI SDK 7 HarnessAgent (Codex) — EXPERIMENTAL; requires a Vercel "
            "sandbox, does NOT run on benchflow's local sandbox (use codex-acp)"
        ),
    )
    for alias in _ALIASES:
        AGENT_ALIASES.setdefault(alias, "ai-sdk-codex")
