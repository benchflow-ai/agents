...
from ai_sdk_harness_claude_code.register import _install_cmd, register
from benchflow.agents.registry import resolve_agent

_SERVER = (Path(__file__).parents[1] / "src" / "ai_sdk_harness_claude_code" / "server.mjs").read_text()


def test_register_wires_claude_code_harness() -> None:
    register()
    cfg = resolve_agent("ai-sdk-claude-code")
    assert cfg.protocol == "acp"
    assert cfg.api_protocol == "anthropic-messages"  # Claude Code = Anthropic Messages API
...
def test_install_pins_canary_harness_and_vercel_sandbox() -> None:
    ...
    assert "@ai-sdk/harness-claude-code@canary" in cmd
    assert "@ai-sdk/sandbox-vercel@canary" in cmd  # bridge-backed → Vercel sandbox


def test_server_is_claude_code_on_vercel_sandbox() -> None:
    assert "createClaudeCode" in _SERVER
