// EXPERIMENTAL — Vercel AI SDK 7 HarnessAgent (Claude Code harness) over ACP.
//
// ⚠️ DOES NOT RUN AS A BENCHFLOW EVAL. The Claude Code harness is bridge-backed: it
...
// of Claude Code, use benchflow's native `claude-agent-acp` agent. This package is shipped for
...
const log = (...a) => process.stderr.write("[ai-sdk-claude-code] " + a.join(" ") + "\n");
...
let sessionId = "ai-sdk-claude-code-1";
...
async function ensureSession() {
  ...
  const harness = createClaudeCode({
    // Claude Code speaks the Anthropic Messages API; auth via ANTHROPIC_API_KEY / AI Gateway.
...
const KIND = { write: "write", read: "read", bash: "bash", edit: "write", str_replace: "write" };
...
async function handle(msg) {
  ...
  try {
    if (method === "initialize")
      send({ jsonrpc: "2.0", id, result: { protocolVersion: 1, agentInfo: { name: "ai-sdk-claude-code", version: "1.0" },
