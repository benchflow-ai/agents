...
from ai_sdk_harness_opencode.register import _install_cmd, _launch_cmd, register
from benchflow.agents.registry import resolve_agent

_SERVER = (
    Path(__file__).parents[1]
    / "src"
    / "ai_sdk_harness_opencode"
    / "server.mjs"
).read_text()
...
def test_register_wires_harness_agent() -> None:
    register()
    cfg = resolve_agent("ai-sdk-opencode")
    assert cfg.name == "ai-sdk-opencode"
    assert cfg.protocol == "acp"
    assert cfg.api_protocol == "openai-completions"
    # Best-effort default provider slot (TODO: verify in next step).
...
def test_aliases_resolve() -> None:
    register()
    for alias in ("ai-sdk-opencode-harness", "opencode-harness"):
        assert resolve_agent(alias).name == "ai-sdk-opencode"


def test_install_pins_harness_and_deploys_server() -> None:
    cmd = _install_cmd()
    assert "@ai-sdk/harness@1.0.6" in cmd
    assert "@ai-sdk/harness-opencode@1.0.6" in cmd
...
def test_server_keeps_integration_invariants() -> None:
    # HarnessAgent + OpenCode harness + local just-bash sandbox template (real disk)
    assert "HarnessAgent" in _SERVER
    assert "@ai-sdk/harness-opencode" in _SERVER
    assert "createOpenCode" in _SERVER
