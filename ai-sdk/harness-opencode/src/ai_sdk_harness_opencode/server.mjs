...
// (OpenCode harness). UNVERIFIED execution model: opencode bridges to a separate
// opencode process, so whether it runs in-process here is not yet confirmed — the
// just-bash local-sandbox template from harness-pi is kept for now (incl. the
// pi-<sessionId> session-dir layout below) pending the next step. Routes the model
// through benchflow's gateway (OPENROUTER_*/OPENAI_* slot = OpenAI-compatible
// chat-completions) so the harness runs inside benchflow's task sandbox on the
// real task files.
//
...
const log = (...a) => process.stderr.write("[ai-sdk-opencode] " + a.join(" ") + "\n");
...
let sessionId = "ai-sdk-harness-1";
...
async function ensureSession() {
  ...
  const harness = createOpenCode({ auth: { customEnv: { OPENROUTER_API_KEY: key, OPENROUTER_BASE_URL: base } }, model: modelId });
...
async function handle(msg) {
  ...
  try {
    if (method === "initialize")
      send({ jsonrpc: "2.0", id, result: { protocolVersion: 1, agentInfo: { name: "ai-sdk-opencode", version: "1.0" },
